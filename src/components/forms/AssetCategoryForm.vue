...
import { assetCategoryRules } from '@/validations/schemas'
import type { AssetCategory } from '@/types'

const props = defineProps<{
  modelValue?: Partial<AssetCategory> | null
}>()
...
const rules = assetCategoryRules
...
<template>
  <form @submit.prevent="onSubmit" class="flex flex-column gap-3">
    <div class="field">
      <label for="acat-name">الاسم <span class="text-red-500">*</span></label>
      <InputText
        id="acat-name"
        v-model="form.name"
        class="w-full mt-1"
        :class="{ 'p-invalid': v$.name.$error }"
        placeholder="اسم فئة الأصل"
        @blur="v$.name.$touch()"
      />
