...
import type { SalesReturn, SalesReturnCreatePayload, PaginatedPayload } from '@/types'

const API_BASE = '/api/sales-returns'

export const useSalesReturnsStore = defineStore('salesReturns', () => {
  const items = ref<SalesReturn[]>([])
  const meta = ref<PaginatedPayload<SalesReturn>['meta'] | null>(null)
  ...
  async function fetchPage(
    ...
    try {
      ...
      if (filters?.sale_invoice_id) params.sale_invoice_id = filters.sale_invoice_id
      ...
      const payload = parsePaginatedResponse<SalesReturn>(rawPayload)
      ...
      error.value = getErrorMessage(e, 'فشل تحميل مرتجعات المبيعات')
      throw e
    } finally {
      loading.value = false
    }
  ...
  async function create(payload: SalesReturnCreatePayload): Promise<SalesReturn> {
    loading.value = true
    error.value = null
    try {
      const { data } = await apiClient.post(API_BASE, payload)
      const created = unwrapPayload<SalesReturn>(data)
  ...
  async function update(id: number, payload: SalesReturnCreatePayload): Promise<SalesReturn> {
    loading.value = true
    error.value = null
    try {
      const { data } = await apiClient.put(`${API_BASE}/${id}`, payload)
      const updated = unwrapPayload<SalesReturn>(data)
  ...
  /** Fetch a single sale invoice by id (for auto-fill). */
  async function fetchSaleInvoice(invoiceId: number) {
    const { data } = await apiClient.get(`/api/sale-invoices/${invoiceId}`)
    return unwrapPayload<import('@/types').SaleInvoice>(data)
  }
