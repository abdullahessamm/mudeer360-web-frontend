...
// Debounced search: refetch when topbar search changes (only on customers list page)
let searchDebounce: ReturnType<typeof setTimeout> | null = null
watch(
  () => dashboardStore.searchQuery,
  (q) => {
    if (route.name !== 'customers') return
...
const formModel = ref<Partial<Customer> | null>(null)

const formTitle = computed(() => (isEdit.value ? 'تعديل العميل' : 'إضافة عميل'))
...
async function onFormSubmit(payload: Partial<Customer>) {
  try {
    if (isEdit.value && editingId.value !== null) {
      await store.update(editingId.value, payload)
    } else {
      await store.create(payload as Omit<Customer, 'id' | 'created_at' | 'updated_at'>)
    }
...
function confirmDelete(row: Customer) {
  confirm.require({
    message: `هل أنت متأكد من حذف العميل "${row.name}"؟`,
    header: 'تأكيد الحذف',
  ...
  <div dir="rtl">
    ...
    <Card>
      <template #content>
        ...
        >
          <i class="pi pi-user-plus text-6xl text-color-secondary"></i>
          <p class="text-color-secondary m-0">لا يوجد عملاء</p>
          <p class="text-sm text-color-secondary m-0">أضف عميلاً جديداً للبدء</p>
          <Button label="إضافة عميل" icon="pi pi-plus" @click="openCreate" />
        </div>
        <DataTable
          v-else
          ...
          <Column header="الإجراءات" style="width: 260px">
            <template #body="{ data }">
              <Button
                ...
                size="small"
                class="p-button-info"
                @click="router.push({ name: 'customer-invoices', params: { id: data.id } })"
              />
              <Button
                label="تعديل"
                icon="pi pi-pencil"
