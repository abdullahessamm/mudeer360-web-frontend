...
// Debounced search: refetch when topbar search changes (only on suppliers list page)
let searchDebounce: ReturnType<typeof setTimeout> | null = null
watch(
  () => dashboardStore.searchQuery,
  (q) => {
    if (route.name !== 'suppliers') return
...
const formModel = ref<Partial<Supplier> | null>(null)

const formTitle = computed(() => (isEdit.value ? 'تعديل المورد' : 'إضافة مورد'))

function openCreate() {
  isEdit.value = false
  editingId.value = null
  formModel.value = { name: '', phone: '', email: '', address: '', notes: '' }
...
async function onFormSubmit(payload: Partial<Supplier>) {
  try {
    if (isEdit.value && editingId.value !== null) {
      await store.update(editingId.value, payload)
    } else {
      await store.create(payload as Omit<Supplier, 'id' | 'created_at' | 'updated_at'>)
    }
...
function confirmDelete(row: Supplier) {
  confirm.require({
    message: `هل أنت متأكد من حذف المورد "${row.name}"؟`,
    header: 'تأكيد الحذف',
...
<template>
  <div dir="rtl">
    <div class="flex justify-content-end mb-4">
      <Button label="إضافة مورد" icon="pi pi-plus" @click="openCreate" />
    </div>
    <Card>
      <template #content>
        ...
        >
          ...
          <p class="text-color-secondary m-0">لا يوجد موردين</p>
          <p class="text-sm text-color-secondary m-0">أضف مورداً جديداً للبدء</p>
          <Button label="إضافة مورد" icon="pi pi-plus" @click="openCreate" />
        </div>
        <DataTable
          v-else
        ...
        >
          ...
          <Column field="address" header="العنوان">
          ...
          <Column header="الإجراءات" style="width: 260px">
            <template #body="{ data }">
              <Button
                ...
                size="small"
                class="p-button-info"
                @click="router.push({ name: 'supplier-invoices', params: { id: data.id } })"
              />
              <Button
                label="تعديل"
                icon="pi pi-pencil"
