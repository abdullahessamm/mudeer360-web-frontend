...
import { useSalesReturnsStore } from '@/stores/salesReturns'
import { useCustomersStore } from '@/stores/customers'
import { useProductsStore } from '@/stores/products'
import type { SalesReturn, SalesReturnCreatePayload, SaleInvoice } from '@/types'

const confirm = useConfirm()
const store = useSalesReturnsStore()
...
function emptyLine(): ReturnFormLine {
  return { product_id: null, quantity: 1, price: 0, discount: 0, tax: 0 }
}
...
async function onInvoiceSelected(invoiceId: number | null) {
  ...
  if (!formModel.value.customer_id) {
    showError('يرجى اختيار العميل أولاً')
    formModel.value.sale_invoice_id = null
    return
  }

  loadingInvoice.value = true
  try {
    const invoice: SaleInvoice = await store.fetchSaleInvoice(invoiceId)
    
    if (invoice.customer_id !== formModel.value.customer_id) {
      showError('رقم الفاتورة المدخل لا يخص العميل المحدد')
      formModel.value.sale_invoice_id = null
      return
    }

    if (invoice?.items?.length) {
      const discountRatio = (invoice.subtotal_amount && invoice.subtotal_amount > 0) 
        ? (invoice.discount_amount || 0) / invoice.subtotal_amount 
        : 0;
      ...
      const itemsToReturn = invoice.items.map((item) => {
        ...
        const itemTax = Number((itemAmountAfterDiscount * (taxPercentage / 100)).toFixed(2));
...
function lineTotal(line: ReturnFormLine): number {
  return Math.max(0, line.price * line.quantity - line.discount + line.tax)
}
...
function onProductSelected(line: ReturnFormLine, productId: number | null) {
  ...
  if (opt) line.price = opt.sale_price ?? 0
}
...
function onQuantityChange(line: ReturnFormLine, newQty: number) {
  if (line._discountRatio !== undefined && line._taxPercentage !== undefined) {
    const itemSubtotal = newQty * line.price;
    line.discount = Number((itemSubtotal * line._discountRatio).toFixed(2));
    const itemAmountAfterDiscount = itemSubtotal - line.discount;
    line.tax = Number((itemAmountAfterDiscount * (line._taxPercentage / 100)).toFixed(2));
  }
}
...
async function submitForm() {
  ...
  if (formModel.value.sale_invoice_id) {
  ...
  const payload: SalesReturnCreatePayload = {
    customer_id: formModel.value.customer_id!,
    sale_invoice_id: formModel.value.sale_invoice_id || null,
...
function confirmDelete(r: SalesReturn) {
  confirm.require({
    message: `هل أنت متأكد من حذف المرتجع ${r.code}؟ سيتم عكس تأثيره على المخزون والرصيد.`,
    header: 'تأكيد الحذف',
  ...
  <div class="page-container" dir="rtl">
    ...
    <div class="page-header">
      <div class="page-header-left">
        <h1 class="page-title">مرتجعات المبيعات</h1>
        <span class="page-subtitle">إدارة بضاعة المرتجعة من العملاء</span>
      </div>
      <Button
    ...
    <div class="summary-cards">
      <div class="summary-card summary-card--blue">
        <i class="pi pi-arrow-left summary-card__icon"></i>
        <div>
    ...
    <div class="table-card">
      ...
      >
        <Column field="code" header="الكود" style="width: 130px" />
    ...
    <Dialog
      ...
      :header="isEdit ? 'تعديل المرتجع' : 'مرتجع مبيعات جديد'"
      modal
      ...
      <div class="return-form-container">
        ...
        <div class="form-section">
          ...
          <div class="form-grid">
            ...
            <div class="form-field">
              <label class="form-label">فاتورة البيع (اختياري)</label>
              <InputNumber
                v-model="formModel.sale_invoice_id"
              ...
              <div class="item-card-body">
                ...
                <div class="item-field">
                  ...
                  <InputNumber 
                    ...
                    class="w-full"
                    @update:model-value="(val) => onQuantityChange(line, val)" 
                  />
                </div>
                <div class="item-field">
                  <label class="item-label">السعر</label>
                  <InputNumber v-model="line.price" :min="0" :min-fraction-digits="0" :max-fraction-digits="4" class="w-full" />
                </div>
                <div class="item-field">
                ...
                <div class="item-field">
                  <label class="item-label">الضريبة</label>
                  <InputNumber v-model="line.tax" :min="0" :min-fraction-digits="0" :max-fraction-digits="4" class="w-full" />
                </div>
...
.summary-card--purple { background: linear-gradient(135deg, #8b5cf6, #6d28d9); }
