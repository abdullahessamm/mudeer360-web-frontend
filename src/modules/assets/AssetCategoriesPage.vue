...
import AssetCategoryForm from '@/components/forms/AssetCategoryForm.vue'
...
watch(
  ...
  (q) => {
    if (route.name !== 'asset-categories') return
...
const formModel = ref<Partial<AssetCategory> | null>(null)
...
function confirmDelete(row: AssetCategory) {
  confirm.require({
  ...
  <div dir="rtl">
    ...
    <Card>
      <template #content>
        ...
        >
          <i class="pi pi-sitemap text-6xl text-color-secondary"></i>
          <p class="text-color-secondary m-0">لا توجد فئات أصول</p>
